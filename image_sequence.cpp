...
namespace csis3700 {
 ...
 if (scale==1.0)   al_draw_bitmap(images[current].image, x, y, 0);
 else 
     al_draw_scaled_bitmap(images[current].image,
                                        0,0,get_unscaled_width(),get_unscaled_height(),
  ...
  int image_sequence::get_unscaled_width() const {
    assert(images.size() > 0);
    return al_get_bitmap_width(images[0].image);
  }
  int image_sequence::get_unscaled_height() const {
    assert(images.size() > 0);
  ...
  int image_sequence::get_width() const {
    assert(images.size() > 0);
    return int(scale*al_get_bitmap_width(images[0].image));
  }

  int image_sequence::get_height() const {
    assert(images.size() > 0);
    return int(scale*al_get_bitmap_height(images[0].image));
  }
